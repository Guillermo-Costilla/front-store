...
const NavBar = () => {
    ...
    const user = useStoreReg((state) => state.user);
          ...
          <div className="relative flex h-16 items-center justify-between">
            <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
              ...
              <button
                ...
                aria-expanded={menuOpen} // Cambia el aria-expanded según el estado
                onClick={toggleMenu} // Añade el evento onClick
              >
                ...
                <svg
                  className={`${menuOpen ? 'hidden' : 'block'} size-6`} // Cambia la clase según el estado
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
                ...
                <svg
                  className={`${menuOpen ? 'block' : 'hidden'} size-6`} // Cambia la clase según el estado
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="1.5"
                  stroke="currentColor"
            ...
            <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
              ...
              <div className="relative ml-3">
                <div>
                  <button
                    type="button"
                    className={`${profileMenuOpen ? 'block' : 'relative'} flex rounded-full bg-gray-800 text-sm focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800 focus:outline-hidden`}
                    id="user-menu-button"
                    aria-expanded={profileMenuOpen} // Cambia el aria-expanded según el nuevo estado
                    aria-haspopup="true"
                ...
                {/* Menú desplegable, mostrar/ocultar basado en el estado del menú */}
                <div
                  ref={menuRef}
                  className={`${profileMenuOpen ? 'block' : 'hidden'} sm:hidden absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 ring-1 shadow-lg ring-black/5 focus:outline-hidden`}
                  role="menu"
